...
function! AS_M_DetectActiveWindow (filename)
    ...
    let active_window = system('osascript -e ''tell application "MacVim" to every window whose (name begins with "'.shortname.'" )''')
...
function! AS_M_SwitchToActiveWindow (active_window)
    call system('osascript -e ''tell application "MacVim" to set index of '.a:active_window.' to 1''')
    call system('osascript -e ''tell application "System Events" to tell process "MacVim" to perform action "AXRaise" of window 1''')
    cal system('open -a "MacVim"')
endfunction
